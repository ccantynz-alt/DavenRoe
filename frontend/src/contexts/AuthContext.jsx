...
const ADMIN_EMAIL = 'admin@marcoreid.ai';
const ADMIN_PASSWORD = 'MarcoReid2026!';
const ADMIN_USER = {
  id: 'admin-001',
  email: ADMIN_EMAIL,
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    const token = localStorage.getItem('marcoReid_token');
    if (!token) {
      setLoading(false);
      return;
    }
    ...
    api.get('/auth/me')
      ...
      .catch(() => {
        localStorage.removeItem('marcoReid_token');
        localStorage.removeItem('marcoReid_onboarded');
  ...
  const login = async (email, password) => {
    ...
    if (email === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
      localStorage.setItem('marcoReid_token', 'admin-local-token');
      localStorage.setItem('marcoReid_onboarded', 'true');
      setUser(ADMIN_USER);
      return ADMIN_USER;
    }
    ...
    localStorage.setItem('marcoReid_token', access_token);
    localStorage.setItem('marcoReid_onboarded', 'true');
