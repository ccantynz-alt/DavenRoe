...
const CATEGORIES = [
  {
    id: 'getting-started', label: 'Getting Started', icon: '🚀', articles: [
      { id: 'gs1', title: 'Setting up your Astra account', time: '3 min', content: 'Welcome to Astra! After creating your account, you\'ll go through a quick onboarding flow where you set your practice name, default jurisdiction, and connect your first bank feed. Astra supports AU, NZ, UK, and US jurisdictions from day one — no add-ons needed. You can manage multiple entities under one subscription.' },
      { id: 'gs2', title: 'Connecting your first bank feed', time: '4 min', content: 'Navigate to Banking > Connect Bank. Astra supports Plaid (US/Canada), Basiq (Australia/NZ), and TrueLayer (UK/EU). Select your bank, enter your credentials securely (we never store them — the provider handles authentication), and your transactions will start flowing within minutes. The AI categoriser will automatically classify 95%+ of transactions.' },
  ...
  },
  {
    id: 'payroll', label: 'Payroll', icon: '💼', articles: [
      ...
      { id: 'pay3', title: 'Leave management', time: '3 min', content: 'Astra tracks annual leave, sick leave, personal leave, and long service leave automatically based on jurisdiction rules. Employees accrue leave each pay run. You can see current balances, approve leave requests, and forecast future liabilities.' },
      { id: 'pay4', title: 'Superannuation and pensions', time: '4 min', content: 'AU: Super Guarantee is 11.5% (2025-26). NZ: KiwiSaver employer contribution is 3% (employee can choose 3-8%). UK: Auto-enrolment minimum is 3% employer + 5% employee. Astra calculates these automatically and generates the payment schedules.' },
    ],
  },
  {
    id: 'tax', label: 'Tax & Compliance', icon: '📊', articles: [
      { id: 'tax1', title: 'How the tax engine works', time: '4 min', content: 'Astra\'s tax engine applies the correct tax rates based on entity jurisdiction, transaction type, and cross-border rules. It handles GST (AU/NZ), VAT (UK), and Sales Tax (US) automatically. For cross-border transactions, the Treaty Engine applies the correct withholding tax rates under 6 bilateral Double Taxation Agreements.' },
      { id: 'tax2', title: 'Preparing a BAS (Australia)', time: '5 min', content: 'Navigate to Tax Filing. Select the entity and period. Astra pre-populates the BAS from your transaction data: GST collected (1A), GST paid (1B), wages (W1-W5), and PAYG withholding. Review each field, validate with our AI checker, and lodge directly to the ATO. Astra flags common errors before you submit.' },
      ...
      { id: 'tax4', title: 'Cross-border tax treaties', time: '5 min', content: 'Astra supports 6 bilateral DTAs: AU-NZ, AU-UK, AU-US, NZ-UK, NZ-US, UK-US. When you create a cross-border transaction (e.g., AU company paying a UK contractor), the Treaty Engine automatically applies the correct withholding tax rate and generates the required documentation.' },
    ],
  },
  {
    id: 'ai', label: 'AI Features', icon: '🤖', articles: [
      { id: 'ai1', title: 'Ask Astra — natural language queries', time: '3 min', content: 'Type any financial question in plain English: "What was our revenue last quarter?", "Show me overdue invoices over $5,000", "Compare this month\'s expenses to the same month last year". Astra understands context, pulls the data, and responds with formatted answers, charts, or tables.' },
  ...
  },
  {
    id: 'forensic', label: 'Forensic Intelligence', icon: '🔍', articles: [
      { id: 'for1', title: 'Benford\'s Law analysis', time: '4 min', content: 'Benford\'s Law predicts the distribution of first digits in financial data. Real data follows a specific pattern — fabricated data usually doesn\'t. Astra tests your transaction data against Benford\'s expected distribution and flags statistically significant deviations. This is used by auditors, the IRS, and forensic accountants worldwide.' },
      { id: 'for2', title: 'Ghost vendor detection', time: '3 min', content: 'Astra scans your vendor list for suspicious patterns: vendors with no ABN/EIN, duplicate bank accounts, vendors matching employee addresses, vendors with only round-number invoices, or vendors that only appear near period-end. Each flag gets a risk score. Review and investigate directly from the alert.' },
  ...
  },
  {
    id: 'reporting', label: 'Reports', icon: '📈', articles: [
      { id: 'rep1', title: 'Standard financial reports', time: '3 min', content: 'Astra generates: Profit & Loss (by period, entity, or consolidated), Balance Sheet (current or comparative), Trial Balance, Cash Flow Statement, General Ledger, Aged Receivables, and Aged Payables. All reports can be exported to PDF or filtered by date range, entity, and account.' },
...
const FAQ = [
  ...
  { q: 'Is my data secure?', a: 'Astra uses bank-level encryption (AES-256 at rest, TLS 1.3 in transit), role-based access control, immutable audit trails with hash-chain verification, and per-entity data isolation. We\'re SOC 2 Type II compliant.' },
  ...
  { q: 'How accurate is the AI categorisation?', a: 'Astra achieves 95%+ accuracy on transaction categorisation, improving over time as it learns from your corrections. Competitors average 60-70%. Transactions below your confidence threshold go to the Review Queue.' },
  { q: 'Can I white-label Astra for my practice?', a: 'Enterprise plans include full white-label branding: your logo, colours, custom domain, and practice name throughout the platform. Go to Enterprise > Branding to configure.' },
];
