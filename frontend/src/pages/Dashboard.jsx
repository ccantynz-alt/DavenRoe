...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-6xl">
      ...
      {/* Money Overview — the 4 things every tradie needs to see */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <MoneyCard
          label="You're Owed"
          value={loading ? null : fmt(stats?.receivable_outstanding || stats?.outstanding_amount)}
        ...
        <MoneyCard
          label="You Owe"
          value={loading ? null : fmt(stats?.bills_owed)}
          sub={stats?.overdue_bills > 0 ? `${stats.overdue_bills} overdue — pay them` : 'Nothing overdue'}
          color={stats?.overdue_bills > 0 ? 'red' : 'green'}
          icon={<OutgoingIcon />}
          link="/bills"
