...
async def update_appointment(appt_id: str, data: dict, user: User = Depends(get_current_user)):
...
async def cancel_appointment(appt_id: str, data: dict = None, user: User = Depends(get_current_user)):
    data = data or {}
...
async def complete_appointment(appt_id: str, data: dict = None, user: User = Depends(get_current_user)):
    data = data or {}
...
async def get_calendar_invite(appt_id: str, user: User = Depends(get_current_user)):
...
async def user_appointments(user_id: str, from_date: str | None = None, to_date: str | None = None, user: User = Depends(get_current_user)):
...
async def upcoming_appointments(user_id: str, days: int = 7, user: User = Depends(get_current_user)):
    appts = scheduler.upcoming(user_id, days)
    return {"appointments": [a.to_dict() for a in appts], "days": days}


@router.get("/entity/{entity_id}")
async def entity_appointments(entity_id: str, user: User = Depends(get_current_user)):
...
async def set_availability(user_id: str, data: dict, user: User = Depends(get_current_user)):
    return scheduler.set_availability(user_id, data)


@router.get("/availability/{user_id}/{on_date}")
async def check_availability(user_id: str, on_date: str, user: User = Depends(get_current_user)):
    return scheduler.get_availability(user_id, on_date)


# Summary
@router.get("/summary")
async def scheduling_summary(user_id: str | None = None, user: User = Depends(get_current_user)):
